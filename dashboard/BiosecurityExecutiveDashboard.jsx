...
const TASKS = [
  ...
  { id: "ext_appr", name: "Согласование документов по заявкам посещений/въезда", icon: FileText, group: "reg", contour: "external", hours: 8.0, cov: 10, freq: "непрерывно", note: "Заявки на посещение и въезд: документы, маршрут, инструктаж",
  ...
  { id: "ext_docs", name: "Разработка и актуализация документов по требованиям ББ и БЗ", icon: FileText, group: "reg", contour: "external", hours: 4.5, cov: 40, freq: "проект", note: "Нормирующие документы службы: от потребности до контроля исполнения", isNew: true,
  ...
  { id: "ext_contract", name: "Контроль исполнения договоров на предмет ББ", icon: BadgeCheck, group: "reg", contour: "external", hours: 3.0, cov: 10, freq: "ежеквартально", note: "Анализ договоров на соответствие требованиям биобезопасности", isNew: true,
  ...
  { id: "ext_audit", name: "Аудит внешних направлений", icon: ClipboardCheck, group: "crit", contour: "external", hours: 11.0, cov: 30, freq: "ежеквартально", note: "Проверка внешних направлений на соответствие требованиям ББ",
  ...
  // ── ВНУТРЕННИЙ КОНТУР (15 процессов, схема процессов и подпроцессов службы) ──
  { id: "int_rep", name: "Отчётность", icon: FileText, group: "reg", contour: "internal", hours: 6.5, cov: 100, freq: "ежедневно", note: "Сбор суточных данных, отклонения по KPI, отчётность для руководства",
    sub: ["Сбор суточных данных и показателей биобезопасности", "Анализ и систематизация отчётности по подразделениям", "Учёт и анализ отклонений по KPI биобезопасности", "Формирование ежемесячных и итоговых отчётов для руководства", "Архивное хранение отчётности и контроль исполнения рекомендаций"] },
  { id: "int_brief", name: "Инструктажи", icon: Megaphone, group: "reg", contour: "internal", hours: 3.0, cov: 100, freq: "еженедельно", note: "Вводные, регулярные и внеплановые инструктажи персонала",
    sub: ["Определение графиков и категорий сотрудников для инструктажей", "Проведение вводных инструктажей для новых сотрудников", "Проведение регулярных повторных инструктажей на рабочих местах", "Проведение внеплановых инструктажей при изменении требований", "Регистрация участников и выдача допусков в журналах учёта"] },
  { id: "int_train", name: "Обучение сотрудников внутренних", icon: GraduationCap, group: "reg", contour: "internal", hours: 4.5, cov: 60, freq: "ежемесячно", note: "Обучение персонала правилам гигиены, санитарии и биобезопасности",
    sub: ["Оценка потребности в обучении по правилам гигиены и санитарии", "Организация учебных модулей и тренингов по биобезопасности", "Проведение лекций и практических занятий для персонала", "Тестирование сотрудников и проверка усвоенных знаний", "Оценка эффективности обучения и актуализация программ"] },
  { id: "int_lab", name: "Анализ лабораторных данных", icon: FlaskConical, group: "reg", contour: "internal", hours: 3.5, cov: 100, freq: "еженедельно", note: "Отбор проб, мониторинг исследований, выявление отклонений",
    sub: ["Контроль отбора проб смывов, воды, кормов и биологического материала", "Анализ/мониторинг лабораторных исследований и испытаний", "Регистрация и систематизация результатов лабораторных анализов", "Экспертный анализ данных и выявление отклонений / угроз", "Формирование выводов, отчётов и оперативных корректирующих мер"] },
  { id: "int_vnd", name: "ВНД: внутренние требования ББ", icon: FileText, group: "reg", contour: "internal", hours: 4.5, cov: 60, freq: "проект", note: "Мониторинг требований, разработка и контроль исполнения СОП", isNew: true,
    sub: ["Мониторинг изменений законодательных и корпоративных требований", "Разработка и актуализация стандартных операционных процедур (СОП)", "Согласование и утверждение внутренних нормативных документов", "Доведение требований ВНД до ответственных подразделений", "Контроль соблюдения регламентов и ведение реестра требований"] },
  { id: "int_verify", name: "Верификация мойки", icon: BadgeCheck, group: "daily", contour: "internal", hours: 6.5, cov: 100, freq: "ежедневно", note: "Контроль качества мойки и дезинфекции оборудования и помещений",
    sub: ["Разработка критериев и стандартов мойки и дезинфекции оборудования", "Визуальная и инструментальная проверка качества процесса мойки", "Отбор смывов на чистоту и микробиологический контроль", "Анализ результатов верификации и оценка эффективности обработки", "Оформление акта верификации мойки и допуск оборудования к работе"] },
  { id: "int_husk", name: "Склады", icon: Layers, group: "daily", contour: "internal", hours: 4.0, cov: 30, freq: "еженедельно", note: "Санитарное состояние складов, условия хранения, дератизация и дезинсекция",
    sub: ["Контроль доступа и санитарного состояния складских помещений", "Проверка условий хранения сырья, кормов и материалов", "Организация мероприятий дератизации и дезинсекции складов", "Соблюдение принципов ротации, инвентаризация и сроки годности", "Ведение складской документации и учётных журналов"] },
  { id: "int_barrier", name: "Дезбарьеры", icon: ShieldCheck, group: "daily", contour: "internal", hours: 9.0, cov: 50, freq: "ежедневно", note: "Техническое состояние, растворы, концентрации, регистрация проезда",
    sub: ["Проверка технического состояния и исправности дезбарьеров", "Приготовление и регулярная замена дезинфицирующих растворов", "Контроль концентрации, объёма и температуры дезсредств", "Регистрация проезда и прохода транспорта/персонала через дезбарьеры", "Техническое обслуживание оборудования и мониторинг эффективности"] },
  { id: "int_atc", name: "Парк АТЦ", icon: Car, group: "crit", contour: "internal", hours: 5.5, cov: 60, freq: "еженедельно", note: "Санобработка и техническое состояние автотранспорта, маршруты движения",
    sub: ["Санитарная обработка, мойка и дезинфекция автотранспорта", "Проверка технического состояния и чистоты машин перед выездом", "Контроль соблюдения маршрутов движения по чистым/грязным зонам", "Инструктаж водителей по правилам биобезопасности на объекте", "Ведение журналов учёта работы парка АТЦ и обработки транспорта"] },
  { id: "int_manure", name: "Пометохранилище", icon: Warehouse, group: "daily", contour: "internal", hours: 2.0, cov: 100, freq: "еженедельно", note: "Герметичность, безопасная выгрузка и хранение, экологический контроль",
    sub: ["Контроль герметичности, целостности и состояния пометохранилища", "Организация процессов безопасной выгрузки, хранения и обеззараживания", "Мониторинг экологических, санитарных и климатических параметров", "Проведение противоэпидемических обработок (против мух, грызунов, запахов)", "Документирование перемещения, хранения и утилизации помёта"] },
  { id: "int_entry", name: "Внутренний входной контроль", icon: DoorOpen, group: "crit", contour: "internal", hours: 5.5, cov: 5, freq: "ежедневно", note: "Контроль поступающих грузов, сырья и тары внутри контура", isNew: true,
    sub: ["Проверка сопроводительной документации на поступающие грузы и сырьё", "Санитарный осмотр и дезинфекция тары, упаковки и транспорта", "Контроль качества поступающих кормов, инвентаря и материалов", "Принятие решения о допуске или отказе во въезде/входе", "Регистрация данных входного контроля в корпоративной системе"] },
  { id: "int_kkz", name: "ККЗ", icon: Package, group: "crit", contour: "internal", hours: 5.5, cov: 0, freq: "ежедневно", note: "Санитарный и технологический контроль комбикормового завода", isNew: true,
    sub: ["Санитарный контроль сырья и компонентов комбикормов", "Контроль технологических режимов термической обработки сырья", "Мониторинг чистоты оборудования, линий производства и помещений ККЗ", "Лабораторный контроль готовой комбикормовой продукции на безопасность", "Ведение технологической и отчётной документации ККЗ"] },
  { id: "int_pest", name: "Внутренний пест-контроль", icon: Bug, group: "daily", contour: "internal", hours: 3.0, cov: 20, freq: "еженедельно", note: "Мониторинг и локальные обработки внутри контура", isNew: true,
    sub: ["Разработка карты точек мониторинга и размещения ловушек", "Регулярный осмотр станций дератизации и средств защиты от вредителей", "Проведение локальных обработок при обнаружении активности вредителей", "Оценка эффективности внутренних мероприятий пест-контроля", "Ведение журнала учёта пест-контроля и составление отчётов"] },
  { id: "int_bio", name: "Биоотходы и перевозка падежа", icon: Siren, group: "crit", contour: "internal", hours: 3.0, cov: 100, freq: "ежедневно", note: "Сбор, упаковка, транспортировка и утилизация биоотходов",
    sub: ["Выявление, безопасный сбор и маркировка биологических отходов", "Правильная упаковка и погрузка падежа в специализированную тару", "Транспортировка падежа спецтранспортом по строгому маршруту", "Утилизация и уничтожение биоотходов на специализированном объекте", "Оформление ветеринарных сопроводительных документов и актов"] },
  { id: "int_audit", name: "Аудиты площадок и территории", icon: ClipboardCheck, group: "crit", contour: "internal", hours: 11.0, cov: 30, freq: "ежемесячно", note: "Плановые и внезапные проверки на требования ББ и санитарии",
    sub: ["Формирование графика плановых и внезапных аудитов биобезопасности", "Проведение инспекций площадок, производственных цехов и территории", "Выявление несоответствий, уязвимостей и зон риска", "Разработка плана корректирующих и предупреждающих мероприятий", "Контроль устранения замечаний и проведение повторных проверок"] },
];
...
function MindMap({ T, accent, hubTitle, hubSub, items, onItem }) {
  ...
  const hasCov = items.some((it) => it.task);
  const PW = 306, PH = hasCov ? 96 : 80, GAP = 15, PAD = 34;
  ...
  return (
    <svg viewBox={`0 0 960 ${H}`} className="relative block w-full">
      ...
      {items.map((it, i) => {
        const p = place(i);
        const cov = it.task ? it.task.cov : null;
        const state = cov === null ? null : cov >= 80 ? T.teal : cov >= 40 ? T.amber : T.red;
        const tinted = cov !== null && cov < 40;
        return (
          <motion.g
            key={it.key}
          ...
          >
            ...
            {tinted && (
              <rect x={p.x} y={p.y} width={PW} height={PH} rx={14} fill={T.red} opacity={0.1} />
            )}
            <rect x={p.left ? p.x + PW - 4 : p.x} y={p.y + 12} width={4} height={PH - 24} rx={2} fill={accent} />
            <foreignObject x={p.x + 16} y={p.y + 5} width={PW - 40} height={PH - 10}>
              <div
                xmlns="http://www.w3.org/1999/xhtml"
                style={{
                  display: "flex", flexDirection: "column", justifyContent: "center", height: "100%",
                  alignItems: p.left ? "flex-end" : "flex-start",
                  fontFamily: "inherit",
              ...
              >
                <span
                  style={{
                    fontSize: 13.5, lineHeight: 1.3, color: T.text, fontWeight: 600,
                    display: "-webkit-box", WebkitLineClamp: cov === null ? 3 : 2, WebkitBoxOrient: "vertical",
                ...
                {cov !== null && (
                  <span
                    style={{
                      marginTop: 6, fontSize: 12, fontWeight: 700, color: state,
                      background: `${state}1f`, borderRadius: 999, padding: "2px 9px",
