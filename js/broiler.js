  ...
  try {
    ...
    if (tag) {
      ...
      if (Array.isArray(data)) {
        embeddedAllCache = { [SECTIONS[0].key]: data };
...
async function loadSectionEntries(sectionKey) {
  try {
    ...
    // встроенные данные как стартовые. Пустой раздел в такой копии — это
    // тоже осмысленный ответ ("отправитель сюда ничего не клал"), поэтому
    // ниже к демо-записям не проваливаемся: получатель должен видеть
    // ровно то, что ему прислали, и ничего сверх этого.
...
// Разметка миниатюры одного элемента (фото или видео) — для полоски
// миниатюр внутри карточки (там всегда только медиа ТЕКУЩЕЙ записи, то
// есть максимум несколько файлов одновременно в DOM).
...
// Облегчённая миниатюра для списка (renderListView) — рендерится сразу
// для ВСЕХ записей раздела. Полноразмерное видео как <video src="..."> не
// вставляем: если в разделе десятки видео, встраивание их dataURL прямо в
// один большой HTML-фрагмент разом может занять сотни МБ/несколько ГБ и
// уронить вкладку. В списке видео показываем как статичную иконку —
// смотреть его целиком можно по клику (лайтбокс), как и раньше.
function listThumbMarkup(item) {
  if (!item) return "";
  if (item.type === "video") {
    return `<div class="list-card-video-placeholder">🎥</div><span class="media-video-badge">▶</span>`;
  }
  return `<img src="${item.src}" alt="" />`;
}
...
function renderListView(list) {
  ...
  els.listView.innerHTML = list
    .map((entry, idx) => {
      // Одна повреждённая запись (например, от очень старой версии
      // дашборда) не должна ронять показ всего списка — пропускаем её
      // и предупреждаем в консоли, а не оставляем список пустым.
      try {
        const media = entry.media || [];
        return `
        <div class="list-card" data-idx="${idx}">
          <div class="list-card-photo" data-role="photo" title="${media.length ? "Открыть целиком" : "Нет фото/видео"}">
            ${media.length ? listThumbMarkup(media[0]) : "📷"}
...
function showIntakeProgress({ index, total, file, percent }) {
  ...
  if (!isVideo) {
    els.dropzoneNote.textContent = `Обработка фото${position}...`;
    return;
  }
  // Пока не пошли кадры, показывать «0%» — только пугать: непонятно,
  // работает оно или встало. Честнее сказать, что идёт подготовка.
  els.dropzoneNote.textContent =
    percent > 0
      ? `🎥 Сжатие видео${position} — ${Math.round(percent * 100)}%. Не закрывайте вкладку.`
      : `🎥 Готовим видео${position} к сжатию... Не закрывайте вкладку.`;
}

// Замерено на практике: перекодирование съедает примерно 2 МБ исходного
// видео в секунду. Точность тут не нужна — нужен честный порядок величины,
// чтобы человек заранее понимал, ждать ему минуту или полчаса, и мог
// разбить загрузку на части.
const VIDEO_COMPRESS_BYTES_PER_SEC = 2 * 1024 * 1024;

function confirmLongIntake(files) {
  const videoBytes = files
    .filter((f) => f.type.startsWith("video/") && f.size > VIDEO_COMPRESS_MIN_BYTES)
    .reduce((sum, f) => sum + f.size, 0);
  if (!videoBytes) return true;

  const minutes = Math.round(videoBytes / VIDEO_COMPRESS_BYTES_PER_SEC / 60);
  if (minutes < 3) return true;

  return confirm(
    `Выбрано видео примерно на ${(videoBytes / 1024 / 1024).toFixed(0)} МБ — сжатие займёт ориентировочно ${minutes} мин.\n\n` +
      "Всё это время вкладку нужно держать открытой. Если это много — нажмите «Отмена» и загрузите файлы частями.\n\nПродолжить?"
  );
}
...
const VIDEO_STALL_TIMEOUT_MS = 25000; // столько ждём следующий кадр при открытой вкладке
...
async function compressVideoFile(file, onProgress) {
  ...
  let encoder = null;
  try {
    // Браузер не всегда сообщает об ошибке: на неподдерживаемом кодеке
    // (скажем, HEVC со свежего айфона) он может просто молча ничего не
    // сделать. Поэтому ждём метаданные с ограничением по времени.
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error("браузер не смог прочитать видео")), VIDEO_METADATA_TIMEOUT_MS);
      ...
      video.onerror = () => {
        clearTimeout(timer);
        reject(new Error("браузер не поддерживает формат этого видео"));
      };
    });
    ...
    await new Promise((resolve, reject) => {
      ...
      // Свёрнутую вкладку браузер перестаёт отрисовывать, и кадры честно
      // не идут — это не зависание, поэтому время в фоне не засчитываем.
      // Но если вкладка на виду, а кадров всё равно нет (застрявший
      // декодер, полуподдерживаемый кодек), ждать вечно нельзя: одна такая
      // запись подвесит всю очередь из сотни файлов. Сдаёмся и сохраняем
      // ролик как есть.
      watchdog = setInterval(() => {
        ...
        if (Date.now() - lastProgressAt > VIDEO_STALL_TIMEOUT_MS) {
          fail(new Error("перекодирование остановилось"));
        }
      }, 2000);

      video.onended = finish;
      video.onerror = () => fail(new Error("сбой воспроизведения при перекодировании"));
