...
// библиотек. Лимит нужен только чтобы не уронить вкладку совсем
// огромным файлом (при чтении в память уходит примерно в 1.3 раза
// больше исходного размера) — сама загрузка при этом может занять
// заметное время.
const MAX_VIDEO_BYTES = 500 * 1024 * 1024; // ~500 МБ

async function fileToMediaItem(file) {
  if (file.type.startsWith("video/")) {
    if (file.size > MAX_VIDEO_BYTES) {
      alert(
        `Видео «${file.name}» слишком большое (${(file.size / 1024 / 1024).toFixed(0)} МБ, лимит ~500 МБ). ` +
          "Снимите более короткий ролик или сожмите его перед загрузкой."
      );
      return null;
    }
