 ...
 * #sectionToggle. Данные хранятся локально в IndexedDB — фото уходят
 * внутрь как dataURL (уменьшенные и пережатые в JPEG, см.
 * compressDataUrl), так что всё работает полностью офлайн, без бэкенда.
 *
 * Почему IndexedDB, а не localStorage: localStorage ограничен ~5-10 МБ
 * на домен — при большом количестве фото это быстро упирается в лимит
 * ("не удалось сохранить"). IndexedDB даёт на порядки больше места
 * (обычно сотни МБ и выше, зависит от свободного места на диске). Старые
 * данные из localStorage (v1-v3) при первом запуске автоматически
 * переносятся в IndexedDB — см. loadEntries/loadReferenceEntries.
 *
 ...
 * вшитыми данными обоих разделов (см.
 * #embeddedData/#embeddedReferenceData и exportShareableCopy ниже) —
...
const state = {
  ...
  discrepancies: [],
  reference: [],
  entries: [], // алиас на discrepancies или reference — см. setSection ниже
  index: 0,
...
const IDB_NAME = "aitas_dashboard";
const IDB_VERSION = 1;
const IDB_STORE = "sections"; // записи вида {key: "discrepancies" | "reference", data: [...]}
...
async function loadEntries() {
  try {
    ...
    if (stored !== null) return normalizeEntries(stored);
...
async function loadReferenceEntries() {
  try {
    const stored = await idbGet("reference");
    if (stored !== null) return normalizeEntries(stored);

    const embedded = readEmbeddedJson("embeddedReferenceData");
    if (embedded) {
      const normalized = normalizeEntries(embedded);
      await idbSet("reference", normalized).catch(() => {});
      return normalized;
    }
...
function saveEntries() {
  const key = state.section === "reference" ? "reference" : "discrepancies";
  idbSet(key, state.entries).catch((err) => {
...
async function exportToPowerPoint() {
  ...
  try {
    ...
    title.addText("aitas mpf — Макинская птицефабрика · Цех инкубации", {
...
const SIDEBAR_COLLAPSED_KEY = "aitas_sidebar_collapsed";
...
function renderListView(list) {
      ...
      return `
        <div class="list-card" data-idx="${idx}">
          <div class="list-card-photo" data-role="photo" title="${photos.length ? "Открыть фото целиком" : "Нет фото"}">
            ...
            ${photos.length ? '<span class="zoom-hint">⤢</span>' : ""}
          </div>
  ...
  els.listView.querySelectorAll(".list-card").forEach((card) => {
    ...
    card.querySelector('[data-role="photo"]').addEventListener("click", () => {
      if (photos.length) openLightbox(photos, 0, entry.location);
      else {
        state.index = idx;
        setViewMode("cards");
      }
...
function renderCard(entry, direction) {
  ...
  card.innerHTML = `
    <div class="card-photo" id="cardPhoto" title="${hasPhotos ? "Нажмите, чтобы посмотреть фото целиком" : "Нажмите, чтобы загрузить фото"}">
      ${
        hasPhotos
          ? `<img src="${entryPhotos[0]}" alt="Фото" />`
  ...
  card.querySelector("#cardPhoto").addEventListener("click", (e) => {
    ...
    if (entryPhotos.length) openLightbox(entryPhotos, 0, entry.location);
...
async function boot() {
  const [discrepancies, reference] = await Promise.all([loadEntries(), loadReferenceEntries()]);
  state.discrepancies = discrepancies;
  state.reference = reference;
  state.entries = state.section === "reference" ? state.reference : state.discrepancies;
  renderSidebar();
  renderView();
}
